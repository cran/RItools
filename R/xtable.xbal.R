...
xtable.xbal <- function(x,caption = NULL, label = NULL, align =c("l",rep(".",ncol(xvardf))),
  ...
  {##By default use decimal alignment, which will require the dcolumn package in latex and an appropriate column definition like:
    ...
    names(xvardf)<-if(!is.null(col.labels)){col.labels} else {
      ##paste(rep(attr(x$vartable,"col.vars")[["strata"]],each=length(attr(x$vartable,"col.vars")[["stat"]])),attr(x$vartable,"col.vars")[["stat"]],sep=".")
      paste("\\multicolumn{1}{c}{",rep(attr(xprint$vartable,"col.vars")[["stat"]],numstrata),"}",sep="")
    }

    ##call xtable on the resulting data.frame
    vartab <- xtable(xvardf,digits=digits,align=align,hline.after=c(0,nrow(xvardf)),...) ##NextMethod("xtable",xvardf)
