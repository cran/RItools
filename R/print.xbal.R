print.xbal <- function (x, which.strata=dimnames(x$results)[["strata"]],
                        ...
                        show.signif.stars=getOption("show.signif.stars"),show.pvals=!show.signif.stars,
                        horizontal=TRUE,...)
{ ##Notes: right now we've decided that you can't print both signif stars and p-values. make a choice.
  ...
  ##Setup digits code: store the preset option and reset it at the end.
  OLDDIGITS<-getOption("digits")
  if (is.null(digits)){
    DIGITS <- max(2, getOption("digits")-4) ##I think the default should be fewer digits: JB.
  } else {DIGITS <- digits}
  options(digits=DIGITS)

  ##Summarize the output array as a flat contingency table
  if (length(theresults) == 0) {
    cat("There is a problem. Probably all of the variables (",
        all.vars(formula(x)),
  ...
  if(show.signif.stars&!show.pvals){
    Signif <- symnum(theresults[,"p",which.strata,drop=FALSE], corr = FALSE, na = FALSE, abbr.colnames=FALSE, ##from print.summary.lm
    ...
    if(horizontal){
      theftab<-ftable(newresults[which.vars,c(which.stats[!(which.stats=="p")],"sig."),which.strata,drop=FALSE],col.vars=c("strata","stat"),row.vars=c("vars"))
      attr(theftab,"col.vars")$stat[attr(theftab,"col.vars")$stat=="sig."]<-""
      if("z" %in% which.stats){attr(theftab,"col.vars")$stat[attr(theftab,"col.vars")$stat=="z"]<-"   z   "}
      thevartab<-theftab
    } else {
      thevartab<-sapply(which.strata,simplify=FALSE,function(x){cbind(as.data.frame(theresults[which.vars,c(which.stats[!(which.stats=="p")]),x])," "=format(Signif[which.vars,,x]))})
    }
    
  }
  if(show.pvals&("p"%in% dimnames(theresults)[["stat"]])){
    if(horizontal){
      theftab<-ftable(theresults[which.vars,which.stats,which.strata,drop=FALSE],col.vars=c("strata","stat"),row.vars=c("vars"))
      thevartab<-theftab
    } else {
      thevartab<-sapply(which.strata,simplify=FALSE,function(x){as.data.frame(theresults[which.vars,which.stats,x])})
    }
  }
  if(show.pvals&!("p"%in% dimnames(theresults)[["stat"]])){print("You need to request p-values when calling xBalance."); stop()} ##irrelevant now.
 ...
 if (!is.null(theoverall)) {
   ...
   if(show.signif.stars){
     ChiSignif<-symnum(theoverall[which.strata,"p.value"], corr = FALSE, na = FALSE, abbr.colnames=NULL, ##from print.summary.lm
                       cutpoints = c(0, 0.001, 0.01, 0.05, 0.1, 1), 
                       symbols = c("***", "** ", "*  ", ".  ", "   "))
